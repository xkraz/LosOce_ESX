...
ESX.Game.GetClosestPedReverse = function(coords, ignoreList)
	local ignoreList      = ignoreList or {}
	local peds            = ESX.Game.GetPedsReverse(ignoreList)
	local closestDistance = -1
	local closestPed      = -1
...
ESX.Game.Utils.DrawText3D = function(x, y, z, text)
	...
	if onScreen then
        ...
        DrawText(_x,_y)
        local factor = (string.len(text)) / 520
        DrawRect(_x, _y + 0.0120, 0.030 + factor , 0.030, 147, 55, 221, 50)
	end
end

ESX.Game.Utils.MiniMapPing = function(x, y, z)
	local Opacity = 250
